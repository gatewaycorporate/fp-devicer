 ...
 */
export class DefaultAttractorModel implements AttractorModel {
 ...
 */
export class FrequencyTableAttractorModel implements AttractorModel {
  private readonly table: FrequencyTable;
  private readonly defaultModel = new DefaultAttractorModel();
  ...
  score(fp: FPDataSet): number {
    ...
    if (this.table.platforms) {
      ...
      const freq = Object.entries(this.table.platforms).find(
        ([key]) => platform.includes(key.toLowerCase())
      )?.[1];
    ...
    if (this.table.languages) {
      ...
      const freq = Object.entries(this.table.languages).find(
        ([key]) => lang === key.toLowerCase() || lang.startsWith(key.toLowerCase() + "-") || key.toLowerCase().startsWith(lang + "-")
      )?.[1];
    ...
    if (this.table.browserFamilies) {
      ...
      const freq = Object.entries(this.table.browserFamilies).find(
        ([key]) => ua.includes(key.toLowerCase())
      )?.[1];
    ...
    if (this.table.hardwareProfiles) {
      ...
      const match = this.table.hardwareProfiles.find(
        (p) => p.concurrency === c && p.memory === m
      );
  ...
  private _defaultPlatformRisk(fp: FPDataSet): number {
    const platform = String(fp.platform ?? "").toLowerCase();
    const ua = String(fp.userAgent ?? "").toLowerCase();
  ...
  private _defaultLanguageRisk(fp: FPDataSet): number {
    const lang = String(fp.language ?? "").toLowerCase();
    return ["en", "en-us", "zh-cn"].includes(lang) ? 1 : 0;
  }

  private _defaultBrowserRisk(fp: FPDataSet): number {
    const ua = String(fp.userAgent ?? "").toLowerCase();
    return ua.includes("chrome/") || ua.includes("firefox/") || ua.includes("safari/") ? 1 : 0;
  }

  private _defaultResolutionRisk(fp: FPDataSet): number {
  ...
  private _defaultHardwareRisk(fp: FPDataSet): number {
    const c = Number(fp.hardwareConcurrency ?? 0);
    const m = Number(fp.deviceMemory ?? 0);
 ...
 * ```
 */
export function createFrequencyTableAttractorModel(table: FrequencyTable): AttractorModel {
