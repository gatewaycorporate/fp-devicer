  ...
  for (let i = 0; i < maxLength; i++) {
    if (sortedArr1[i] !== undefined && sortedArr2[i] !== undefined) {
      ...
      if (Array.isArray(sortedArr1[i]) && Array.isArray(sortedArr2[i])) {
        ...
        const subData = compareDatasets(sortedArr1[i] as FPDataSet, sortedArr2[i] as FPDataSet, max_depth - 1);
        fields += subData[0] - 1; // Subtract 1 for the index itself
        matches += subData[1];
      }
...
export function compareDatasets(data1: FPDataSet, data2: FPDataSet, max_depth: number = 5): [number, number] {
  let fields = 0;
  let matches = 0;
  ...
  for (const key in data1) {
    if (data1[key] !== undefined && data2[key] !== undefined) {
      ...
      if (
        ...
        const subData = compareDatasets(data1[key] as FPDataSet, data2[key] as FPDataSet, max_depth - 1);
