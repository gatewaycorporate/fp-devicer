...
export class DeviceManager {
        ...
        if (!baseResult) {
            ...
            if (this.lshIndex) {
                ...
                if (extraIds.length > 0) {
                    const extraCandidates = extraIds.map((deviceId) => ({
                        deviceId,
            ...
            const incomingFonts = Array.isArray(incoming.fonts)
                ? incoming.fonts
                : [];
            if (incomingFonts.length > 0) {
                for (const cand of candidates) {
                    if (cand.deviceId === deviceId)
                        continue; // skip matched device
                    const candHistory = await this.adapter.getHistory(cand.deviceId, 1);
                    if (!candHistory.length)
                        continue;
                    const candFonts = Array.isArray(candHistory[0].fingerprint.fonts)
                        ? candHistory[0].fingerprint.fonts
                        : [];
     ...
     */
    async analyzeDeviceDrift(deviceId, incoming, options) {
