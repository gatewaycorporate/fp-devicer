 ...
 */
export function computeDeviceDrift(incoming, history, options = {}) {
    ...
    if (sorted.length >= 2) {
        for (const field of FIELD_PATHS) {
            ...
            for (let i = 0; i < sorted.length - 1; i++) {
                const v1 = sorted[i].fingerprint[field];
                const v2 = sorted[i + 1].fingerprint[field];
    ...
    for (const field of FIELD_PATHS) {
        ...
        for (const snap of sorted) {
            const histValue = snap.fingerprint[field];
    ...
    const allFields = [];
    for (const field of FIELD_PATHS) {
        if (fieldDeviation[field] === undefined)
            continue;
    ...
    const suspiciousFields = allFields
